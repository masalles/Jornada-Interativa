...
const RegisterComponent = {
  render(container) {
    ...
    container.innerHTML = `
      <div class="register-screen min-h-screen flex flex-col items-center justify-center text-center px-4 py-10 relative z-10">
        <div class="max-w-md w-full bg-white bg-opacity-95 rounded-lg shadow-lg overflow-hidden backdrop-filter backdrop-blur-sm">
          <div class="bg-secondary py-4">
            <h2 class="text-2xl font-bold text-white">Cadastrar</h2>
          </div>
          ...
          <div class="p-6">
            ...
            <form id="register-form" class="space-y-4">
              <div class="text-left">
                ...
                <input type="text" id="name" name="name" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-secondary" required>
              </div>

              <div class="text-left">
                <label for="username" class="block text-gray-700 text-sm font-bold mb-2">Nome de Usuário</label>
                <input type="text" id="username" name="username" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-secondary" required>
              </div>

              <div class="text-left">
                <label for="password" class="block text-gray-700 text-sm font-bold mb-2">Senha</label>
                <input type="password" id="password" name="password" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-secondary" required>
              </div>

              <div class="text-left">
                <label for="confirm-password" class="block text-gray-700 text-sm font-bold mb-2">Confirmar Senha</label>
                <input type="password" id="confirm-password" name="confirm-password" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-secondary" required>
              </div>

              <div>
                <button type="submit" class="w-full bg-secondary hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:ring-2 focus:ring-yellow-500 transition duration-300">
                  Cadastrar
                </button>
              </div>
            </form>
            ...
            <div class="mt-4 text-sm">
              <p>Já tem uma conta? <a href="#" id="go-to-login" class="text-secondary hover:text-yellow-600">Faça login</a></p>
            </div>
          </div>
        </div>
  ...
  handleRegister() {
    ...
    const result = AuthService.register(username, password, name);

    if (result.success) {
      console.log('Registration successful');

      // Show success message briefly before redirecting
      errorElement.textContent = 'Cadastro realizado com sucesso! Redirecionando...';
      errorElement.classList.remove('hidden');
      ...
      // Show error message
      errorElement.textContent = result.message || 'Erro ao fazer cadastro. Tente novamente.';
      errorElement.classList.remove('hidden');
    }
  },
  ...
  addAnimationStyles() {
    // Add enhanced animation styles for the register screen
    if (!document.getElementById('register-animations')) {
      const styleEl = document.createElement('style');
      styleEl.id = 'register-animations';
      styleEl.textContent = `
        /* Subtle fade-in animation for the entire register screen */
        .register-screen {
